...
def allocate(request):
    ...
    pbs = ProductBuyer.objects.all()

    for trav in travellers:
        for pb in pbs:
            if((pb.product.location==trav.origin) and(pb.buyer.location==trav.destination)):
...
def loadProducts(request):
    ...
    for el in le:
        ...
        Product.objects.create(name= name, price= price, location= location, imageUrl= imageUrl)
